...
int deb_p(int pass_func)
{
    ...
    for (size_t Al_p = 0; Al_p < eas_count; Al_p++)
    {
        if (n == eas_pas[Al_p])
        {
            fprintf(stderr, "Error: Password is too easy to guess. Please choose a stronger password.\n");
            exit(2);
        }
    }
    return n;
...
int  change_password(int pass_func)
{
    ...
    if(old_p != pass_func)
    {
        fprintf(stderr, "Error: Old password does not match current password.\n");
        exit(8);
    }
...
char* deb_u()
{
    ...
    for (size_t i = 0; i < ulen; i++) {
        if ((u_n[i] < 'a' || u_n[i] > 'z') && (u_n[i] < 'A' || u_n[i] > 'Z'))
        {
            if ((u_n[i] < '0' || u_n[i] > '9') && u_n[i] != '_'  && u_n[i] != '\n' && u_n[i] != '@')
            {
                fprintf(stderr, "Error: Username contains invalid characters. Only alphanumeric characters, underscores and '@' are allowed.\n");
                exit(4);
            }
        }
    }
